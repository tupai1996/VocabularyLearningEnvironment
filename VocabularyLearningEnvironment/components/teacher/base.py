...
class Teacher:
    ...
    def __init__(
        self, material: List[TeachingItem], planner: Planner, context: PlanningContext
    ):
        # Assertions
        is_list = isinstance(material, list)
        contains_teaching_items = all(
            isinstance(item, TeachingItem) for item in material
        )
        if not is_list or not (material) or not contains_teaching_items:
            raise TypeError(
                "material must be a non-empty list of TeachingItem instances"
            )
